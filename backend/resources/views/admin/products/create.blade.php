...
@section('header', 'Add New Product / Note')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold text-slate-900">Create New Note / Product</h2>
            <p class="text-xs text-slate-500 mt-0.5">Fill in the details below to add a new CBSE note or product to your store</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.products.store') }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div class="sm:col-span-2">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name') }}" required
                        placeholder="e.g. CBSE Class 10 Handwritten Science Notes (All Chapters)"
                ...
                <div>
                    ...
                    <input type="text" id="slug" name="slug" value="{{ old('slug') }}"
                        placeholder="cbse-class-10-handwritten-science-notes"
                ...
                <div>
                    ...
                    <input type="number" id="stock" name="stock" value="{{ old('stock', 50) }}" min="0"
                        class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-brand-500 focus:bg-white transition-colors">
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="4"
                        placeholder="Describe the note's topics, exam benefits, syllabus coverage..."
                        class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-brand-500 focus:bg-white transition-colors">{{ old('description') }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" step="0.01" id="price" name="price" value="{{ old('price') }}" required placeholder="49.99"
                ...
                <div>
                    <label for="old_price" class="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1.5">
                        Original / Old Price (Rs) <span class="text-slate-400">(Optional for discount badge)</span>
                    </label>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 flex items-center pl-3 text-sm font-bold text-slate-400">Rs</span>
                        <input type="number" step="0.01" id="old_price" name="old_price" value="{{ old('old_price') }}" placeholder="99.00"
                ...
                <div>
                    <label for="badge" class="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1.5">
                        Badge Text <span class="text-slate-400">(e.g. 'BESTSELLER', 'HOT DEAL', 'CBSE 2026')</span>
                    </label>
                    <input type="text" id="badge" name="badge" value="{{ old('badge') }}" placeholder="BESTSELLER"
                        class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-brand-500 focus:bg-white transition-colors">
                </div>
                ...
                <div>
                    ...
                    <select id="badge_type" name="badge_type" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-brand-500 focus:bg-white">
                        <option value="best" {{ old('badge_type') == 'best' ? 'selected' : '' }}>Amber / Gold (Best Seller)</option>
                        <option value="sale" {{ old('badge_type') == 'sale' ? 'selected' : '' }}>Rose / Red (Sale / Discount)</option>
                        <option value="new" {{ old('badge_type') == 'new' ? 'selected' : '' }}>Emerald / Green (New Arrival)</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-sm space-y-5">
            <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                ...
                <button type="button" onclick="addImageField()" class="text-xs font-bold text-brand-600 hover:text-brand-700 flex items-center gap-1">
                    <i class="fa-solid fa-plus-circle"></i> Add Another Image
                </button>
            </div>

            <p class="text-xs text-slate-500">
                Paste any direct image URL or <strong class="text-slate-700">Google Drive share link</strong> (e.g. <code class="bg-slate-100 px-1 py-0.5 rounded text-[11px]">https://drive.google.com/file/d/ID/view</code>). The CMS automatically converts Google Drive links into high-speed direct images.
            </p>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <textarea id="features" name="features" rows="5"
                        placeholder="Complete Syllabus Coverage (2025-26)&#10;Color diagrams & flowchart summary&#10;Solved Previous Year Questions&#10;Formula cheat sheet included"
                        class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:border-brand-500 focus:bg-white font-mono transition-colors">{{ old('features') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea id="table_of_contents" name="table_of_contents" rows="5"
                        placeholder="Chapter 1: Chemical Reactions & Equations&#10;Chapter 2: Acids, Bases & Salts&#10;Chapter 3: Metals & Non-metals&#10;Chapter 4: Carbon & its Compounds"
                        class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:border-brand-500 focus:bg-white font-mono transition-colors">{{ old('table_of_contents') }}</textarea>
                </div>

                <div class="sm:col-span-2">
                    <label for="direct_link" class="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1.5">
                        Direct Download / PDF Drive Link <span class="text-slate-400">(Sent to customer after purchase)</span>
                    </label>
                    <input type="text" id="direct_link" name="direct_link" value="{{ old('direct_link') }}"
        ...
        <div class="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-sm space-y-4">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <label class="flex items-center gap-2.5 p-3 rounded-xl border border-slate-200 bg-slate-50 cursor-pointer hover:bg-slate-100/60">
                    <input type="checkbox" name="is_bestseller" value="1" {{ old('is_bestseller') ? 'checked' : '' }} class="w-4 h-4 text-brand-600 rounded">
                    <span class="text-xs font-bold text-slate-800">Bestseller</span>
                </label>

                <label class="flex items-center gap-2.5 p-3 rounded-xl border border-slate-200 bg-slate-50 cursor-pointer hover:bg-slate-100/60">
                    <input type="checkbox" name="is_sale" value="1" {{ old('is_sale') ? 'checked' : '' }} class="w-4 h-4 text-brand-600 rounded">
                    <span class="text-xs font-bold text-slate-800">On Sale</span>
                </label>

                <label class="flex items-center gap-2.5 p-3 rounded-xl border border-slate-200 bg-slate-50 cursor-pointer hover:bg-slate-100/60">
                    <input type="checkbox" name="is_new" value="1" {{ old('is_new') ? 'checked' : '' }} class="w-4 h-4 text-brand-600 rounded">
